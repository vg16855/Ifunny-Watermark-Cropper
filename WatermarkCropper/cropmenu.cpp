...
cropMenu::cropMenu(QWidget* parent) : QWidget(parent), ui(new Ui::cropMenu) {
    ui->setupUi(this);
...
void cropMenu::loadImages(QStringList fileList) {
    ...
    QPointer<QProgressDialog> imageLoadProgress = new QProgressDialog(
        "Loading Images", "Abort", 0, fileList.size(), this);
    ...
    for (int i = 0; i < fileList.length(); i++) {
        ...
        if (!image.load(fileList[i])) {
            std::cout << "invalid image: " << fileList[i].toStdString()
                      << std::endl;
        ...
        label->setPixmap(bitmap.scaled(100, 100, Qt::KeepAspectRatio,
                                       Qt::SmoothTransformation));
        label->setAlignment(Qt::AlignCenter);
        ui->imageGridView->addWidget(label, j, k,
                                     {Qt::AlignTop, Qt::AlignLeft});
        connect(label, &Clickable::clicked, this,
                [=]() { emit(labelClicked(fileList[i])); });

        // Connects label to a checkbox
        QPointer<QCheckBox> checkBox =
            new QCheckBox(ui->imageGridView->itemAt(validCounter)->widget());
        ...
        connect(checkBox, &QCheckBox::stateChanged, [=](bool checked) {
            ...
            ui->cropNumber->setText(
                QString("Images to Crop:  %1").arg(numChecked));
        });
...
void cropMenu::detectWatermark() {
    ...
    QPointer<QProgressDialog> imageComparison = new QProgressDialog(
        "Identifying Watermarks", "Abort", 0, validImages.size(), this);
    ...
    for (int i = 0; i < validImages.size(); i++) {
        ...
        if (settings.value("compare/algo").toInt() == HIST_INDEX) {
            ...
            cv::Mat imageToCheck =
                createHistogram(std::get<FILE_PATH>(validImages[i]));
            if (imageToCheck.empty()) {
                std::cout
                    << "Error Creating Histogram, try changing the file name"
                    << std::endl;
            ...
            if (score < histogramThreshold) {
                QCheckBox* checkBox;
                checkBox = ui->imageGridView->itemAt(i)
                               ->widget()
                               ->findChild<QCheckBox*>();
            ...
            // Calculates score based on similarity(lower = better)
            float score =
                compareImage(std::get<FILE_PATH>(validImages[i]), matWatermark);
            if (score == -1) {
                std::cout
                    << "Error Calculating Norm, try changing the file name"
                    << std::endl;
                imageComparison->setValue(i + 1);
                ui->cropNumber->setText(
                    QString("Images to Crop:  %1").arg(numChecked));
...
cv::Mat cropMenu::createHistogram(QString fileName) {
    ...
    cv::Mat histImg;
    cv::calcHist(&hsvImg, 1, channels, cv::Mat(), histImg, 2, histSize, ranges,
                 true, false);
...
float cropMenu::compareImage(QString fileName, cv::Mat watermark) {
    ...
    cv::Mat corner =
        image(cv::Rect(image.cols - 140, image.rows - 20, 140, 20));

    // Compare two matrices
    if (corner.rows > 0 && corner.rows == watermark.rows && corner.cols > 0 &&
        corner.cols == watermark.cols) {
        // Calculate the L2 relative error between images.
        double errorL2 = norm(corner, watermark, cv::NORM_L2);
        // Convert to a reasonable scale, since L2 error is summed across all
        // pixels of the image.
...
void cropMenu::labelClicked(QString filePath) {
    ...
    ui->previewLabel->setPixmap(pix.scaled(width, height, Qt::KeepAspectRatio,
                                           Qt::SmoothTransformation));
}
...
void cropMenu::saveImages(QDir directory) {
    // Create progress bar
    QPointer<QProgressDialog> imageSaveProgress = new QProgressDialog(
        "Saving Images", "Abort", 0, validImages.size(), this);
    imageSaveProgress->setWindowModality(Qt::WindowModal);
    imageSaveProgress->show();
    connect(imageSaveProgress, SIGNAL(canceled()), this,
            SLOT(myCustomCancel()));
    int failed = 0;
...
void cropMenu::noImageMessage() {
    QMessageBox info;
    info.setText(
        "No watermarks are being cropped, "
...
void cropMenu::on_saveAs_clicked() {
    ...
    QDir directory = QFileDialog::getExistingDirectory(
        this, tr("Open Directory"), savePath,
