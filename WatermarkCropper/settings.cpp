...
settings::settings(QWidget *parent) : QDialog(parent), ui(new Ui::settings) {
    ...
    QSettings settings;
    std::cout << "Settings Filepath: " << settings.fileName().toStdString()
              << std::endl;
    ...
    if (settings.contains("compare/algo")) {
        ui->comparisonComboBox->setCurrentIndex(
            settings.value("compare/algo").toInt());
    } else {
...
void settings::on_comparisonComboBox_currentIndexChanged() {
    ...
    std::cout << "Combobox Value: " << ui->comparisonComboBox->currentIndex()
              << std::endl;
}
...
void settings::on_histSpinBox_valueChanged() {
    ...
    std::cout << "Histogram Spinbox Value: " << ui->histSpinBox->value()
              << std::endl;
}
...
void settings::on_normSpinBox_valueChanged() {
    ...
    std::cout << "Normalization Spinbox Value: " << ui->normSpinBox->value()
              << std::endl;
}
...
void settings::on_dialogButton_accepted() {
    ...
    std::cout << "Comparison Combo Box Saved Value: "
              << settings.value("compare/algo").toInt() << std::endl;
    std::cout << "Histogram Spinbox Saved Value: "
              << settings.value("thresh/hist").toDouble() << std::endl;
    std::cout << "Normalization Spinbox Saved Value: "
              << settings.value("thresh/norm").toDouble() << std::endl;
}

// Cancels all changes and resets to what they were when the settings menu was
// first started
